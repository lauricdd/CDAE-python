...
class DAE():
    ...
    def do_not_pretrain(self):
        with tf.compat.v1.variable_scope("SDAE_Variable"):
    ...
    def do_pretrain(self,pretrain_input,epoch,batch_size,learning_rate,dropout,corruption_level):
        ...
        '''
        pre_W1 = tf.compat.v1.get_variable(name=("pre_W1"+str(self.itr)), shape=[self.n_visible, self.n_hidden], dtype=tf.float32,initializer=tf.compat.v1.random_normal_initializer())
        pre_b1 = tf.compat.v1.get_variable(name=("pre_b1"+str(self.itr)), shape=[self.n_hidden], dtype=tf.float32,
                                 initializer=tf.compat.v1.random_normal_initializer())

        pre_W2 = tf.compat.v1.get_variable(name=("pre_W2"+str(self.itr)), shape=[self.n_hidden, self.n_visible], dtype=tf.float32,initializer=tf.compat.v1.random_normal_initializer())
        pre_b2 = tf.compat.v1.get_variable(name=("pre_b2"+str(self.itr)), shape=[self.n_visible], dtype=tf.float32,
                                 initializer=tf.compat.v1.random_normal_initializer())

        X = tf.compat.v1.placeholder("float", [None, self.n_visible])
        mask = tf.compat.v1.placeholder("float", [None,self.n_visible],name='mask')
        ...
        with tf.compat.v1.Session() as sess:
            ...
            pretrain_input = tf.convert_to_tensor(value=pretrain_input)
